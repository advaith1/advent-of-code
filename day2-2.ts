const text = `Game 1: 1 red, 5 blue, 1 green; 16 blue, 3 red; 6 blue, 5 red; 4 red, 7 blue, 1 green
...
for (const line of lines) {
	...
	for (const set of sets) {
		...
		for (let color of set.split(', ')) {
			...
			map[color.split(' ')[1]] = +color.split(' ')[0]
		}
		console.log(map)
		if (map.red > maxRed) maxRed = map.red	
		if (map.green > maxGreen) maxGreen = map.green	
		if (map.blue > maxBlue) maxBlue = map.blue	
	}
	console.log(`game ${gameNum}: ${maxRed} red, ${maxGreen} green, ${maxBlue} blue`)
